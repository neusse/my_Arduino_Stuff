...
// 
//#############################################################################
void incrementAlarm(void) { // Advance alarm time by one minute
...
// 
//#############################################################################
void decrementAlarm(void) { // Retard alarm time by one minute
...
// 
//#############################################################################
void TurnOffAlarm(void) { // This cancels the alarm when it is going off (or alarmSnoozed).
...
void checkButtons(void) {
    ...
    if (milliTemp >= NextButtonCheck) // Typically, go through this every 20 ms.
    {
        ...
        if (buttonMonitor) { // If any buttons have been down in last ButtonCheckInterval
            ...
            if ((buttonMonitor & a5_alarmSetBtn) && ((buttonStateLast & a5_alarmSetBtn) == 0)) { // If Alarm Set button was just depressed
        ...
        if (modeShowMenu && holdDebounce) { // Button behavior, when in Config menu mode:
            ...
            if (((buttonMonitor & a5_plusBtn) == 0) && (buttonStateLast & a5_plusBtn)) { // The "+" button has just been released.
                optionValue = 1;
                UpdateEE = 1;
                RedrawNow = 1;
            }

            if (((buttonMonitor & a5_minusBtn) == 0) && (buttonStateLast & a5_minusBtn)) { // The "-" Button has just been released.
                optionValue = -1;
                UpdateEE = 1;
                RedrawNow = 1;
            }
        } else { // Button behavior, when NOT in Config menu mode:
            ...
            if ((buttonMonitor & a5_alarmSetBtn) && (buttonMonitor & a5_timeSetBtn) && holdDebounce) {
                ...
                {
                    if (modeShowDateViaButtons == 0) { // Display date
...
// 
//#############################################################################
void DisplayWord(char WordIn[], unsigned int duration) { // Usage: DisplayWord("ALARM", 500);
    modeShowText = 1;
            ...
            if (c == 'S') {
                ...
                { // COMMAND: B0, Set Parameters
                    ...
                    if (c == '2') {
                        ...
                        if (c == '1') { // Load altnernate number set
                    ...
                    if (c2 <= '9') { // if we're here, c2 is <= '9', c2 != 0, and c2 != '0'.
...
void updateNightLight(void) {
    if (NightLightType == 4) { // "Sleep" mode
        unsigned int temp = 0;
...
//#IFDEF TEMP 
//            else if (DisplayMode &32U ) {
//              DisplayWord("TEMP ", 500);
//            }
//#ENDIF 
            else
                DisplayWord(" NONE", 500);
        ...
        if ((DisplayMode > 3) && (DisplayMode < 32)) {
            ...
            } else { // Alternate display modes: "Time and ... "
                if (DisplayMode & 4)
...
void AdjDayMonthYear(int8_t AdjDay, int8_t AdjMonth, int8_t AdjYear) {
    ...
    if (dayTemp > monthDays[moTemp - 1])
        if (AdjDay > 0) { // Roll over day-of-month to 1, if explicitly requesting increase in date.
            dayTemp = 1;
        }
    else { // Otherwise, we should "truncate" the date to last day of month.
        dayTemp = monthDays[moTemp - 1];
    }
...
void setDSToffset(uint8_t mode) {
    ...
    if (mode == 2) { // Auto DST
        if (DST_updated)
            return; // already done it once today
...
void TimeDisplay(byte DisplayModeLocal, byte forceUpdateCopy) {
    ...
    if ((DisplayModeLocal <= 4) || (DisplayModeLocal == 20)) { // Normal time display OR Alarm time display
    ...
    } else if (DisplayModeLocal == 36) // FLW - FIVE LETTER WORD mode
    {
        ...
        if (forceUpdateCopy) {
            if (DisplayModeLocalLast != 36) { // Pick new display word, but only when first entering mode 36.
