...
// processMenuItems
//#############################################################################
void processMenuItems(byte forceUpdate)
{
    ...
    case AMPM24HRMenuItem: // Hour mode: 12Hr / 24 Hr
        if (optionValue != 0)
        {
            if (HourMode24)
                HourMode24 = 0;
    ...
    case NightLightMenuItem: // Night Light
    ...
    case AlarmToneMenuItem: // Alarm Tone: 2
    ...
    case SoundTestMenuItem: // Alarm Test: 3
        DisplayWord(" +/- ", 500);
        if (optionValue != 0)
        {
            if (alarmNow == 0)
    ...
    case AltModeMenuItem:
        // Alternate with seconds or date:
        ...
        // if (TimeDisplay & 32): Alternate date with temperature
    ...
    else
    {
        ...
        if ((DisplayMode > 3) && (DisplayMode <= 32))
        {
            ...
            { // Alternate display modes: "Time and ... "
                ...
                else if (DisplayMode & 32)
                    TimeDisplay(99, forceUpdate); // if Bit 32 is set:  Show RTC Temperature
...
void AdjDayMonthYear(int8_t AdjDay, int8_t AdjMonth, int8_t AdjYear)
{
    ...
    const uint8_t monthDays[] = {
        31, 29, 31, 30, 31, 30,
        31, 31, 30, 31, 30, 31}; // february can have 29 days
...
void TimeDisplay(byte DisplayModeLocal, byte forceUpdateCopy)
{
    ...
    if ((hour(tNow) == 0) && (minute(tNow) == 0) && (second(tNow) == 0))
    { // MIDNIGHT!
        DST_updated = false;
    ...
    { // Normal time display OR Alarm time display
        ...
        if (DisplayModeLocal & 1) // Seconds Spinner Mode
        {
            ...
            byte segs1[] = {
                ...
                8, 8, 9, 9, 4, 4, 14, 14}; // wbp
            byte segs2[] = {
                0, 16, 0, 5, 0, 17, 0, 8,
                0, 9, 0, 4, 0, 14, 0, 15};    // wbp
...
//                        a5loadOSB_DP("01000", a5_brightLevel); // non-flashing separator
                    if (SecNow & 1)
                        ...
                        a5loadOSB_DP("00000", a5_brightLevel); // non-flashing separator
                }
    ...
    else if (DisplayModeLocal == 99) // show RTC chip temperature
    {
